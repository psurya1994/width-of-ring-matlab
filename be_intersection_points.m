...
%     for theta = 70
        ...
        disp(['Processing ' num2str(i) 'th segment: theta = ' num2str(theta)])
        search_point = center;
    ...
    if(bool_plot == true)
        ...
        for j = 1:length(points)
            plot(points{j}(:,2),points{j}(:,1), 'g.', 'MarkerSize', 15)
            plot(points{j}(:,2),points{j}(:,1), 'b', 'LineWidth', 2)
        end
        plot(center(2),center(1),'r.', 'MarkerSize', 15)
    end
